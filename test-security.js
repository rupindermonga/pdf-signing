/**
 * CertaDocs QA + Security Dynamic Test Suite
...
async function multipartReq(urlPath, fields, fileField, fileBuffer, fileName, cookie) {
  return new Promise((resolve) => {
    const boundary = '----CertaDocsTest' + crypto.randomBytes(8).toString('hex');
    let body = '';
...
async function run() {
  console.log('\n\x1b[1m══════════════════════════════════════════\x1b[0m');
  console.log('\x1b[1m  CertaDocs QA + Security Audit\x1b[0m');
  ...
  const otpResp = await req('POST', '/api/auth/send-otp', { email: 'test@certadocs.local' });
  ...
  const wrongOtp = await req('POST', '/api/auth/verify-otp', { email: 'test@certadocs.local', code: '000000' });
  wrongOtp.status === 400 ? log('PASS', 'Wrong OTP rejected', '400') : log('FAIL', 'Wrong OTP rejected', `Got ${wrongOtp.status}`);

  // Verify with correct code
  const realOtp = otpData.devOtp || latestOtpFor('test@certadocs.local');
  const rightOtp = await req('POST', '/api/auth/verify-otp', { email: 'test@certadocs.local', code: realOtp, name: 'Test User' });
  ...
  // Replay same OTP
  const replayOtp = await req('POST', '/api/auth/verify-otp', { email: 'test@certadocs.local', code: realOtp });
  ...
  const dbResp = await req('GET', '/data/certadocs.db');
  (dbResp.status === 404) ?
  ...
  // (it is hardcoded as 'certadocs' in server.js — flag it)
  serverCode.includes("passphrase: 'certadocs'") ?
